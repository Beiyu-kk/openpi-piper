...
class DataConfig:
    ...
    # Optional LeRobot video decoder backend. Useful when torchcodec cannot load system FFmpeg libraries.
...
class LeRobotPiperDataConfig(DataConfigFactory):
    """Data config for single-right-arm Piper datasets in LeRobot format."""

    binarize_gripper_outputs: bool = False
    delta_gripper_action: bool = False
    ...
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        ...
        data_transforms = _transforms.Group(
            ...
            outputs=[piper_policy.PiperOutputs(binarize_gripper=self.binarize_gripper_outputs)],
        )

        # By default, keep compatibility with existing Piper checkpoints:
        # joints are trained as deltas, while the continuous gripper stays absolute.
        # New experiments can opt into delta gripper actions with a separate train config.
        delta_action_mask = _transforms.make_bool_mask(7) if self.delta_gripper_action else _transforms.make_bool_mask(6, -1)
        data_transforms = data_transforms.push(
            inputs=[_transforms.DeltaActions(delta_action_mask)],
            outputs=[_transforms.AbsoluteActions(delta_action_mask)],
        )
...
_CONFIGS = [
    ...
    TrainConfig(
        name="pi05_piper_right_book_v5_lora_all_delta",
        model=pi0_config.Pi0Config(
            pi05=True,
            action_dim=32,
            action_horizon=15,
