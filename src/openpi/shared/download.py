...
def _is_google_cloud_gsutil(gsutil: str) -> bool:
    try:
        result = subprocess.run([gsutil, "version"], check=False, capture_output=True, text=True, timeout=10)
    except (OSError, subprocess.SubprocessError):
        return False
    return result.returncode == 0 and "Google Cloud Storage" in result.stdout
